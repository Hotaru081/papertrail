...
if (isset($_POST['submit'])) {
    ...
    $fetch_query = "SELECT * FROM `details` WHERE pno='$pno'";
    ...
    if ($result) {
        ...
        if ($row['pno'] != $pno || $row['particulars'] != $particulars || $row['papertype'] != $papertype || $row['amount'] != $amount || $row['taon'] != $taon || $row['resp_center'] != $resp_center || $row['account_type'] != $account_type || $row['Edate'] != $Edate || $row['status'] != $status || $row['office'] != $office ||
            ...
            // Proceed with update
            $update_query = "UPDATE `details` SET pno='$pno', particulars='$particulars', papertype='$papertype', amount='$amount', taon='$taon', resp_center='$resp_center', account_type='$account_type', Edate='$Edate', status='$status', office='$office', comments='$comments', Eno='$Eno', PRno='$PRno', PRdate='$PRdate', POno='$POno', POdate='$POdate', person='$person', admin='$admin', timestamp=NOW() WHERE pno='$pno'";
    ...
    if ($data_changed || (!empty($filenames) && !$data_changed)) {
        ...
        if (mysqli_query($conn, $activity_query)) {
            // Update the future table if data changed or files uploaded
            $future_update_query = "UPDATE `future` SET particulars='$particulars', amount='$amount', taon='$taon', resp_center='$resp_center', account_type='$account_type' WHERE pno='$pno'";
            
            if (mysqli_query($conn, $future_update_query)) {
                // Redirect if successful
                header("Location: admin.php");
                exit();
            } else {
                echo "Error updating future table: " . mysqli_error($conn);
            }
        } else {
