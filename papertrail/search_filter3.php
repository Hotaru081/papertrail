...
<a href="filter3.php"><button class="back-buttonn">Back</button></a>
...
if (isset($_GET['query'])) {
    ...
    if ($resultWithLimit && mysqli_num_rows($resultWithLimit) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($resultWithLimit)) {
            ...
            echo "<tr class='$rowClass'>";
            echo "<td>
                    <a href='update_search.php?id={$row['id']}' style='font-size: 13px; margin-left: 27px; text-decoration: none; color: black; font-weight: bold'>Update <i class='fas fa-edit fa-sm' style='margin-left: 7px; color:#2667a0'></i></a>
                    ...
                    <div class='dropdown-content' style='font-size: 13px; display: none;'>
                        <a href='checklist_search.php?id={$row['id']}'>Checklist</a>    
...
echo "</table></div></div>";
        ...
        // Display left arrow
        if ($currentPage > 1) {
            echo "<a href='search_filter3.php?query=$searchTerm&page=" . ($currentPage - 1) . "'>&lt;</a>";
        } else {
            // If on the first page, make the "Previous" button go to the last page
            echo "<a href='search_filter3.php?query=$searchTerm&page=$totalPages'>&lt;</a>";
        }
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
            ...
            if ($i == $currentPage) {
                echo "<li class='current'><a href='search_filter3.php?query=$searchTerm&page=$i' >$i</a></li>";
            } else {
                echo "<a href='search_filter3.php?query=$searchTerm&page=$i'>$i</a>";
            }
        ...
        if ($currentPage < $totalPages) {
            ...
            echo "<a href='search_filter3.php?query=$searchTerm&page=1'>&gt;</a>";
        }
...
document.addEventListener('keydown', function(event) {
      if (event.key === 'Escape') {
        ...
        window.location.href = 'filter3.php'; // Redirect to admin.php when Escape key is pressed
      }
    });
    </script>   
</body>
