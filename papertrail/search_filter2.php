...
if (isset($_GET['query'])) {
    ...
    if ($resultWithLimit && mysqli_num_rows($resultWithLimit) > 0) {
        ...
        // Display left arrow
        if ($currentPage > 1) {
            echo "<a href='search_filter2.php?query=$searchTerm&page=" . ($currentPage - 1) . "'>&lt;</a>";
        } else {
            // If on the first page, make the "Previous" button go to the last page
            echo "<a href='search_filter2.php?query=$searchTerm&page=$totalPages'>&lt;</a>";
        }
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
            ...
            if ($i == $currentPage) {
                echo "<li class='current'><a href='search_filter2.php?query=$searchTerm&page=$i' >$i</a></li>";
            } else {
                echo "<a href='search_filter2.php?query=$searchTerm&page=$i'>$i</a>";
            }
        ...
        if ($currentPage < $totalPages) {
            ...
            echo "<a href='search_filter2.php?query=$searchTerm&page=1'>&gt;</a>";
        }
...
document.addEventListener('keydown', function(event) {
      if (event.key === 'Escape') {
        ...
        window.location.href = 'filter2.php'; // Redirect to admin.php when Escape key is pressed
      }
    });
    </script>   
</body>
