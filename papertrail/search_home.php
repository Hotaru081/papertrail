...
if (isset($_GET['query'])) {
    ...
    if ($resultWithLimit && mysqli_num_rows($resultWithLimit) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($resultWithLimit)) {
            ...
            echo "<tr class='$rowClass'>";
            echo "<td>
                <a href='attachment_home.php?pno={$row['pno']}' style='font-size: 13px; margin-left: 18px; text-decoration: none; color: black; font-weight: bold;'>Attachment<i class='fas fa-paperclip fa-sm' style='margin-left: 5px; color: #2667a0'></i></a>
...
echo "<td style='text-align: center; font-size: 13px'>{$row['person']}</td>";
echo "<td style='text-align: center; font-size: 13px'>{$row['admin']}</td>";
...
echo "</table></div></div>";
        ...
        // Display left arrow
        if ($currentPage > 1) {
            echo "<a href='search_home.php?query=$searchTerm&page=" . ($currentPage - 1) . "'>&lt;</a>";
        } else {
            // If on the first page, make the "Previous" button go to the last page
            echo "<a href='search_home.php?query=$searchTerm&page=$totalPages'>&lt;</a>";
        }
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
            ...
            if ($i == $currentPage) {
                echo "<li class='current'><a href='search_home.php?query=$searchTerm&page=$i' >$i</a></li>";
            } else {
                echo "<a href='search_home.php?query=$searchTerm&page=$i'>$i</a>";
            }
        ...
        if ($currentPage < $totalPages) {
            ...
            echo "<a href='search_home.php?query=$searchTerm&page=1'>&gt;</a>";
        }
